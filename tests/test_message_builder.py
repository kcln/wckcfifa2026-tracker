...
def test_daily_recap_omits_group_standings():
    # The group stage is over; the recap no longer carries the frozen tables.
    group_tables = {"A": [
        {"team": "Brazil", "played": 3, "points": 7, "gd": 2, "gf": 2, "ga": 0}]}
    text = mb.daily_recap("2026-06-28", [], group_tables, qualified={"Brazil"})
    assert "Group Standings" not in text and "<code>" not in text
...
def test_post_match_hit_shows_100_percent():
    ...
    assert "<b>Germany</b> 7 - 1 Curaçao" in body     # winner bolded
...
def test_daily_recap_results_sorted_by_kickoff():
    ...
    # A 2-2 a is a draw (no bold); B beat b 1-0 so B is bolded.
    assert body.index("A 2 - 2 a") < body.index("<b>B</b> 1 - 0 b")


def test_post_match_penalty_line_flanks_dash_and_bolds_winner():
    match = {"home": "Germany", "away": "Paraguay",
             "prediction": {"home": 0.5, "draw": 0.25, "away": 0.25},
             "result": {"home_goals": 1, "away_goals": 1, "winner": "Paraguay",
